...
TEST_CASE("Unterminated string - detected newline", "[lexer]") {
  std::string filename = "";
  std::string source = R"("AB
CD")";
  Lexer lexer{"", 0, "", 1, 1};
  lexer.filename = filename;
  lexer.source = source;
...
TEST_CASE("Unterminated string - reached EOF", "[lexer]") {
  std::string filename = "";
...
TEST_CASE("Incorrect unicode escape sequence", "[lexer]") {
  std::string filename = "";
